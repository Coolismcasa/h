...
const FALLBACK_PRODUCTS=[
  {id:'p1',name:'Classic Red Tee',cat:'shirts',price:2490,oldPrice:3200,tag:'New',images:['https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?w=700&q=80'],desc:'Relaxed-fit tee from heavyweight 320 GSM cotton.',sizes:['S','M','L','XL','XXL'],colors:[{name:'Red',hex:'#E33A3A'},{name:'Navy',hex:'#0B1A30'}],fabric:'100% Combed Cotton',care:'Machine wash cold',sku:'CLM-SH-001',inStock:true,stock:50,lowStock:5},
  {id:'p2',name:'Royal Oxford Shirt',cat:'shirts',price:4990,oldPrice:null,tag:null,images:['https://images.unsplash.com/photo-1596755094514-f87e34085b2c?w=700&q=80'],desc:'Crisp Oxford weave with mother-of-pearl buttons.',sizes:['S','M','L','XL'],colors:[{name:'White',hex:'#F9F8F6'},{name:'Sky',hex:'#A8C4E0'}],fabric:'Egyptian Cotton',care:'Machine wash warm',sku:'CLM-SH-002',inStock:true,stock:30,lowStock:5},
  {id:'p3',name:'Pleated Wide-Leg Trousers',cat:'pants',price:5890,oldPrice:6990,tag:'Bestseller',images:['https://images.unsplash.com/photo-1594633312681-425c7b97ccd1?w=700&q=80'],desc:'Tailored single front pleat, wide flowing leg.',sizes:['30','32','34','36','38'],colors:[{name:'Brown',hex:'#6B5442'},{name:'Charcoal',hex:'#2A2A2A'}],fabric:'Poly-wool blend',care:'Dry clean only',sku:'CLM-PT-001',inStock:true,stock:25,lowStock:5},
  {id:'p4',name:'Relaxed Linen Pants',cat:'pants',price:4290,oldPrice:null,tag:null,images:['https://images.unsplash.com/photo-1624378439575-d8705ad7ae80?w=700&q=80'],desc:'Breathable pure linen, soft elasticated back.',sizes:['30','32','34','36'],colors:[{name:'Sand',hex:'#D6C7A8'},{name:'Sage',hex:'#9CAE93'}],fabric:'European Linen',care:'Machine wash cold',sku:'CLM-PT-002',inStock:true,stock:40,lowStock:5},
  {id:'p5',name:'Moto Leather Jacket',cat:'jackets',price:18900,oldPrice:22500,tag:'Limited',images:['https://images.unsplash.com/photo-1551028719-00167b16eac5?w=700&q=80'],desc:'Classic asymmetrical biker in full-grain leather.',sizes:['S','M','L','XL'],colors:[{name:'Black',hex:'#0E0E0E'},{name:'Espresso',hex:'#3A2C20'}],fabric:'Sheep leather',care:'Wipe clean',sku:'CLM-JK-001',inStock:true,stock:8,lowStock:3},
  {id:'p6',name:'Navy Bomber Jacket',cat:'jackets',price:11900,oldPrice:null,tag:null,images:['https://images.unsplash.com/photo-1591047139829-d91aecb6caea?w=700&q=80'],desc:'Lightweight nylon bomber with ribbed cuffs.',sizes:['S','M','L','XL','XXL'],colors:[{name:'Navy',hex:'#0B1A30'},{name:'Olive',hex:'#4A5240'}],fabric:'Nylon shell',care:'Machine wash cold',sku:'CLM-JK-002',inStock:true,stock:20,lowStock:5},
  {id:'p7',name:'Oversized Navy Hoodie',cat:'hoodies',price:6490,oldPrice:7990,tag:'Bestseller',images:['https://images.unsplash.com/photo-1556821840-3a63f95609a7?w=700&q=80'],desc:'Dropped shoulder, boxy fit, drapes perfectly.',sizes:['S','M','L','XL','XXL'],colors:[{name:'Navy',hex:'#131F3A'},{name:'Charcoal',hex:'#3A3A3A'}],fabric:'85% Cotton/15% Poly',care:'Machine wash cold',sku:'CLM-HD-001',inStock:true,stock:60,lowStock:5},
...
async function loadCatalog(){
  try{const cs=await db.collection('categories').get();if(!cs.empty){const loaded={};cs.forEach(doc=>{const d=doc.data();loaded[doc.id]={label:d.label||doc.id,sub:d.sub||'',img:d.img||''};});CATEGORIES={...FALLBACK_CATS,...loaded};}else CATEGORIES={...FALLBACK_CATS};}
  catch(e){CATEGORIES={...FALLBACK_CATS};}
  try{const s=await db.collection('products').get();if(!s.empty){PRODUCTS=s.docs.map(doc=>{const d=doc.data();return{id:doc.id,name:d.name||'',cat:d.cat||'shirts',price:Number(d.price)||0,oldPrice:d.oldPrice?Number(d.oldPrice):null,tag:d.tag||null,images:Array.isArray(d.images)?d.images:(d.img?[d.img]:[]),desc:d.desc||'',sizes:Array.isArray(d.sizes)?d.sizes:['S','M','L','XL'],colors:Array.isArray(d.colors)?d.colors:[{name:'Navy',hex:'#0B1A30'}],fabric:d.fabric||'',care:d.care||'',sku:d.sku||'',inStock:d.inStock!==false,stock:d.stock??50,lowStock:d.lowStock??5,_ts:d.createdAt?.seconds||0};});}else PRODUCTS=[...FALLBACK_PRODUCTS];}
  catch(e){PRODUCTS=[...FALLBACK_PRODUCTS];}
}

/* RENDER CATEGORIES */
function renderCategoryTiles(){const g=document.getElementById('catGrid');if(!g)return;g.innerHTML=Object.keys(CATEGORIES).map(k=>{const c=CATEGORIES[k];return `<a href="category.html?cat=${k}" class="cat-tile"><img class="cat-img" src="${c.img}" alt="${c.label}" loading="lazy" onerror="this.style.background='#ddd';this.style.display='none'"><div class="cat-inner"><h3>${c.label}</h3><p>${c.sub}</p></div></a>`;}).join('');}
...
function toggleWishlist(id){const i=wishlist.indexOf(id);if(i>-1)wishlist.splice(i,1);else wishlist.push(id);saveWishlist();renderProducts(currentFilter,currentSort);const dw=document.getElementById('detailWishBtn');if(dw&&currentDetail.product&&currentDetail.product.id===id)dw.classList.toggle('wished',wishlist.includes(id));toast(wishlist.includes(id)?'Added to wishlist':'Removed from wishlist');}
function renderWishlistDrawer(){const body=document.getElementById('wishlistBody');if(!body)return;const items=PRODUCTS.filter(p=>wishlist.includes(p.id));if(!items.length){body.innerHTML=`<div class="cart-empty"><svg viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg><p>Your wishlist is empty.</p><a href="index.html#shop" class="btn btn-line" style="margin-top:16px">Browse Products</a></div>`;return;}body.innerHTML=items.map(p=>`<div class="cart-item"><div class="ci-thumb">${cartVisualHTML(p)}</div><div class="ci-info"><h4>${p.name}</h4><div class="p">${money(p.price)}</div><button class="ci-remove" data-wish-remove="${p.id}">Remove</button></div></div>`).join('');}
...
/* PRODUCT DETAIL */
function openDetail(id){const p=PRODUCTS.find(x=>x.id===id);if(!p)return;currentDetail={product:p,size:p.sizes[0],color:p.colors[0].name,qty:1,images:p.images&&p.images.length?p.images:[]};currentImageIndex=0;
const media=document.getElementById('detailMedia'),thumbs=document.getElementById('detailThumbs'),counter=document.getElementById('imgCounter'),prev=document.getElementById('imgPrev'),next=document.getElementById('imgNext');
...
if(prev)prev.style.display=total>1?'grid':'none';
if(next)next.style.display=total>1?'grid':'none';
if(thumbs){if(total>1)thumbs.innerHTML=currentDetail.images.map((img,i)=>`<div class="thumb ${i===0?'active':''}" data-thumb="${i}"><img src="${img}" alt="${p.name} ${i+1}"></div>`).join('');else thumbs.innerHTML='';}
const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v;};
set('detailCat',getCat(p.cat).label);set('detailName',p.name);
...
set('detailDesc',p.desc||'');set('detailFabric',p.fabric||'—');set('detailCare',p.care||'—');set('detailSku',p.sku||'—');
...
if(sw){if(p.stock!==undefined&&p.stock<=5&&p.stock>0){sw.textContent=`Only ${p.stock} left in stock!`;sw.style.display='block';}else if(p.stock===0){sw.textContent='Out of stock';sw.style.display='block';}else sw.style.display='none';}
loadReviews(p.id);updateDetailRating(p.id);
document.getElementById('detailModal').classList.add('show');document.body.style.overflow='hidden';}
function showImageIndex(idx){if(!currentDetail.images.length)return;const total=currentDetail.images.length;currentImageIndex=((idx%total)+total)%total;const img=currentDetail.images[currentImageIndex];document.getElementById('detailMedia').innerHTML=`<img src="${img}" alt="Product">`;document.getElementById('imgCounter').textContent=`${currentImageIndex+1} / ${total}`;document.querySelectorAll('.detail-thumbs .thumb').forEach((t,i)=>t.classList.toggle('active',i===currentImageIndex));}
async function updateDetailRating(pid){const el=document.getElementById('detailRating');if(!el)return;try{const s=await db.collection('reviews').where('productId','==',pid).get();if(s.empty){el.innerHTML='<span class="stars-display">☆☆☆☆☆</span> <span style="color:var(--text-soft);font-size:.8rem">No reviews yet</span>';return;}const revs=s.docs.map(d=>d.data());const avg=revs.reduce((a,r)=>a+(r.rating||0),0)/revs.length;const full=Math.round(avg);const stars='★'.repeat(full)+'☆'.repeat(5-full);el.innerHTML=`<span class="stars-display">${stars}</span> <span style="color:var(--text-soft);font-size:.82rem">${avg.toFixed(1)} · ${revs.length} review${revs.length>1?'s':''}</span>`;}catch(e){}}
async function loadReviews(pid){currentReviewProduct=pid;selectedStar=0;document.querySelectorAll('#starPicker span').forEach(s=>s.classList.remove('active'));const list=document.getElementById('reviewsList');if(!list)return;list.innerHTML=`<p style="color:var(--text-soft);font-style:italic;text-align:center;padding:20px;font-size:.9rem">Loading reviews…</p>`;try{const s=await db.collection('reviews').where('productId','==',pid).get();if(s.empty){list.innerHTML=`<div class="reviews-empty">No reviews yet — be the first!</div>`;return;}const revs=s.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));list.innerHTML=revs.map(r=>{const date=r.createdAt?.toDate?r.createdAt.toDate().toLocaleDateString():'';const stars='★'.repeat(r.rating||0)+'☆'.repeat(5-(r.rating||0));return `<div class="review-item"><div class="rv-head"><span class="rv-name">${r.userName||'Anonymous'}</span><span class="rv-stars">${stars}</span></div><p class="rv-text">${r.text||''}</p>${date?`<p class="rv-date">${date}</p>`:''}</div>`;}).join('');}catch(e){list.innerHTML=`<div class="reviews-empty">Could not load reviews.</div>`;}}
...
function performSearch(q){const r=document.getElementById('searchResults');if(!r)return;const term=q.trim().toLowerCase();if(!term){r.classList.remove('show');return;}const m=PRODUCTS.filter(p=>p.name.toLowerCase().includes(term)||(p.desc||'').toLowerCase().includes(term)||(p.sku||'').toLowerCase().includes(term)||getCat(p.cat).label.toLowerCase().includes(term));if(!m.length){r.innerHTML=`<div class="search-empty">No products match "${q}"</div>`;r.classList.add('show');return;}r.innerHTML=m.map(p=>`<div class="search-result" data-search-id="${p.id}">${(p.images&&p.images[0])?`<img src="${p.images[0]}" alt="${p.name}">`:`<div class="product-thumb-ph">${p.name.charAt(0)}</div>`}<div class="sr-info"><div class="sr-name">${p.name}</div><div class="sr-price">${money(p.price)}</div></div></div>`).join('');r.classList.add('show');}
...
async function placeOrder(user,formData){
  ...
  }catch(e){console.error(e);toast('Could not place order. Try again.');}
}
...
try{const snap=await db.collection('orders').where('uid','==',user.uid).get();const orders=snap.docs.map(doc=>({id:doc.id,...doc.data()})).sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));renderProfileOrders(orders);}catch(e){console.error(e);}
renderProfileWishlist();}
function renderProfileOrders(orders){const c=document.getElementById('ordersContainer');if(!c)return;const ce=document.getElementById('ordersCount'),te=document.getElementById('ordersTotal');if(ce)ce.textContent=orders.length;if(te)te.textContent=money(orders.reduce((a,o)=>a+(o.total||0),0));if(!orders.length){c.innerHTML=`<div class="empty-state"><svg viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><path d="M3 6h18"/><path d="M16 10a4 4 0 0 1-8 0"/></svg><h3>No orders yet</h3><p>Your order history will appear here.</p><a href="index.html#shop" class="btn btn-silver">Start Shopping</a></div>`;return;}
const steps=['pending','confirmed','dispatched','delivered'];const labels=['Placed','Confirmed','Dispatched','Delivered'];
c.innerHTML=orders.map(o=>{const date=o.createdAt?.toDate?o.createdAt.toDate().toLocaleDateString():'—';const st=o.status||'pending';let idx=steps.indexOf(st);if(st==='cancelled')idx=-1;const bc=st==='delivered'?'badge-yes':(st==='pending'?'badge-info':(st==='cancelled'?'badge-no':'badge-info'));const tp=idx>=0?(idx/(steps.length-1))*90:0;return `<div class="order-card"><div class="order-card-head"><div><h4>Order ${o.orderId}</h4><p class="oc-date">${date} · ${o.itemCount||0} item(s)</p></div><div style="text-align:right"><div class="oc-total">${money(o.total)}</div><span class="${bc}">${st}</span></div></div><div class="order-track"><div class="order-track-fill" style="width:${tp}%"></div>${steps.map((s,i)=>{const cls=i<=idx?(i===idx?'current':'done'):'';return `<div class="track-step ${cls}"><div class="track-dot">${i+1}</div><span>${labels[i]}</span></div>`;}).join('')}</div><div class="order-card-actions"><button class="link-btn" data-view-order="${o.id}">View Details →</button></div></div>`;}).join('');
c.querySelectorAll('[data-view-order]').forEach(b=>b.addEventListener('click',()=>showOrderDetail(b.dataset.viewOrder,orders)));}
...
function updateImagePreview(slot,src){const p=document.getElementById('prev-'+slot);if(!p)return;if(src){p.innerHTML=`<img src="${src}" alt="Preview" onerror="this.style.display='none'">`;p.classList.add('has-image');}}
function openProductForm(id){const m=document.getElementById('productModal');if(!m)return;if(!Object.keys(CATEGORIES).length)CATEGORIES={...FALLBACK_CATS};fillCategoryDropdown();const f=document.getElementById('productForm');f.reset();document.getElementById('productModalTitle').textContent=id?'Edit Product':'Add New Product';document.getElementById('productModalSub').textContent=id?'Update the details below':'Fill in the details below';
[0,1,2].forEach(i=>{const p=document.getElementById('prev-'+i);if(p){p.innerHTML=`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg><span>Image ${i+1}</span>`;p.classList.remove('has-image');}const inp=document.getElementById('p-img'+(i+1));if(inp)inp.value='';});
if(id){const p=PRODUCTS.find(x=>x.id===id);if(p){document.getElementById('p-name').value=p.name;document.getElementById('p-category').value=p.cat;document.getElementById('p-price').value=p.price;document.getElementById('p-old').value=p.oldPrice||'';document.getElementById('p-tag').value=p.tag||'';document.getElementById('p-sizes').value=(p.sizes||[]).join(', ');document.getElementById('p-sku').value=p.sku||'';document.getElementById('p-colors').value=(p.colors||[]).map(c=>`${c.name}:${c.hex}`).join(', ');document.getElementById('p-desc').value=p.desc||'';document.getElementById('p-fabric').value=p.fabric||'';document.getElementById('p-care').value=p.care||'';document.getElementById('p-instock').checked=p.inStock!==false;document.getElementById('p-stock').value=p.stock??50;document.getElementById('p-lowstock').value=p.lowStock??5;const imgs=p.images||[];for(let i=0;i<3;i++){const inp=document.getElementById('p-img'+(i+1));if(inp&&imgs[i]){const fn=imgs[i].includes('/')?imgs[i].split('/').pop():imgs[i];inp.value=fn;updateImagePreview(i,imgs[i]);}}}f.dataset.editId=id;}else{f.dataset.editId='';document.getElementById('p-stock').value=50;document.getElementById('p-lowstock').value=5;}
m.classList.add('show');}
function closeProductForm(){document.getElementById('productModal').classList.remove('show');}
function parseColors(str){if(!str)return[];return str.split(',').map(s=>s.trim()).filter(Boolean).map(pair=>{const[n,h]=pair.split(':').map(s=>s.trim());return{name:n||'Default',hex:h||'#333333'};});}
function parseImagesFromSlots(baseUrl){const imgs=[];for(let i=1;i<=3;i++){const v=document.getElementById('p-img'+i)?.value.trim();if(v)imgs.push(v.startsWith('http')?v:baseUrl+v);}return imgs;}
async function saveProduct(e){e.preventDefault();const f=document.getElementById('productForm');const editId=f.dataset.editId;
const name=document.getElementById('p-name').value.trim();const cat=document.getElementById('p-category').value;const newCat=document.getElementById('p-newcat').value.trim();const price=Number(document.getElementById('p-price').value);const oldPrice=document.getElementById('p-old').value?Number(document.getElementById('p-old').value):null;const tag=document.getElementById('p-tag').value||null;const sizes=document.getElementById('p-sizes').value.split(',').map(s=>s.trim()).filter(Boolean);const sku=document.getElementById('p-sku').value.trim();const colors=parseColors(document.getElementById('p-colors').value);const desc=document.getElementById('p-desc').value.trim();const fabric=document.getElementById('p-fabric').value.trim();const care=document.getElementById('p-care').value.trim();const inStock=document.getElementById('p-instock').checked;const stock=Number(document.getElementById('p-stock').value)||0;const lowStock=Number(document.getElementById('p-lowstock').value)||5;
const ok=[setErr(document.getElementById('p-name'),name.length<2),setErr(document.getElementById('p-price'),!price||price<0)].every(Boolean);if(!ok)return toast('Please fill required fields');
const baseUrl=window.location.origin+window.location.pathname.replace(/[^/]*$/,'')+'images/';const images=parseImagesFromSlots(baseUrl);
let finalCat=cat;if(newCat){const slug=newCat.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');finalCat=slug;try{await db.collection('categories').doc(slug).set({label:newCat,sub:'New collection',img:images[0]||'',createdAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true});CATEGORIES[slug]={label:newCat,sub:'New collection',img:images[0]||''};}catch(e){console.warn(e);}}
...
try{if(editId){await db.collection('products').doc(editId).update(data);toast('Product updated');}else{data.createdAt=firebase.firestore.FieldValue.serverTimestamp();await db.collection('products').add(data);toast('Product added');}closeProductForm();loadProducts();}catch(e){console.error(e);toast('Could not save: '+(e.code||e.message));}}

/* ADMIN ORDERS */
async function loadOrders(){const t=document.getElementById('ordersTbody');if(!t)return;t.innerHTML=`<tr><td colspan="9" class="table-empty">Loading orders…</td></tr>`;try{const s=await db.collection('orders').orderBy('createdAt','desc').get();allOrders=s.docs.map(d=>({id:d.id,...d.data()}));renderOrdersTable(allOrders);updateAdminStats();}catch(e){t.innerHTML=`<tr><td colspan="9" class="table-empty">Error: ${e.message}</td></tr>`;}}
function renderOrdersTable(list){const t=document.getElementById('ordersTbody');if(!t)return;if(!list.length){t.innerHTML=`<tr><td colspan="9" class="table-empty">No orders yet.</td></tr>`;return;}t.innerHTML=list.map((o,i)=>{const date=o.createdAt?.toDate?o.createdAt.toDate().toLocaleDateString():'—';const st=o.status||'pending';const c=st==='delivered'?'badge-yes':(st==='cancelled'?'badge-no':'badge-info');return `<tr><td>${i+1}</td><td><b>${o.orderId||'—'}</b></td><td>${date}</td><td>${o.fullName||'—'}</td><td>${o.phone||'—'}</td><td>${o.city||'—'}</td><td><b>${money(o.total)}</b></td><td><span class="${c}">${st}</span></td><td><button class="link-btn" data-view-order="${o.id}">View</button></td></tr>`;}).join('');
...
/* ADMIN USERS */
async function loadUsers(){const t=document.getElementById('usersTbody');if(!t)return;t.innerHTML=`<tr><td colspan="9" class="table-empty">Loading users…</td></tr>`;try{const s=await db.collection('users').orderBy('createdAt','desc').get();allUsers=s.docs.map(d=>({id:d.id,...d.data()}));renderUsersTable(allUsers);updateAdminStats();}catch(e){t.innerHTML=`<tr><td colspan="9" class="table-empty">Error: ${e.message}</td></tr>`;}}
function renderUsersTable(list){const t=document.getElementById('usersTbody');if(!t)return;if(!list.length){t.innerHTML=`<tr><td colspan="9" class="table-empty">No users yet.</td></tr>`;return;}t.innerHTML=list.map((u,i)=>{const c=u.createdAt?.toDate?u.createdAt.toDate().toLocaleDateString():'—';const v=u.emailVerified?'<span class="badge-yes">Yes</span>':'<span class="badge-no">No</span>';const n=`${u.firstName||''} ${u.lastName||''}`.trim()||'—';return `<tr><td>${i+1}</td><td>${u.email||'—'}</td><td>${n}</td><td>${u.phone||'—'}</td><td>${u.city||'—'}</td><td>${u.provider||'—'}</td><td>${v}</td><td>${c}</td><td><button class="link-btn" data-view-user="${u.uid||u.id}">View</button></td></tr>`;}).join('');
...
(async function boot(){
  loadLocalWishlist();loadLocalCart();await loadCatalog();
  if(page==='home'){renderCategoryTiles();renderFooterCats();renderFilterChips();renderProducts('all','featured');
    const f=document.getElementById('filters');if(f)f.addEventListener('click',e=>{const chip=e.target.closest('.chip');if(!chip)return;f.querySelectorAll('.chip').forEach(c=>c.classList.remove('active'));chip.classList.add('active');currentFilter=chip.dataset.filter;renderProducts(currentFilter,currentSort);});
...
document.addEventListener('click',e=>{
  ...
  const tabBtn=e.target.closest('.tabs button');if(tabBtn){switchTab(tabBtn.dataset.tab);return;}
  const wishBtn=e.target.closest('[data-wish]');if(wishBtn){e.stopPropagation();toggleWishlist(wishBtn.dataset.wish);return;}
  const quickBtn=e.target.closest('[data-quick]');if(quickBtn){e.stopPropagation();const p=PRODUCTS.find(x=>x.id===quickBtn.dataset.quick);if(!p)return;if(!p.inStock||p.stock<=0)return toast('Out of stock');addToCart(p,p.sizes[0],p.colors[0].name,1);const orig=quickBtn.textContent;quickBtn.textContent='Added ✓';quickBtn.classList.add('added');setTimeout(()=>{quickBtn.textContent=orig;quickBtn.classList.remove('added');},1400);return;}
  const card=e.target.closest('.card');if(card&&card.dataset.id){openDetail(card.dataset.id);return;}
  const inc=e.target.closest('[data-cart-inc]');if(inc){const f=cart.find(i=>lineKey(i)===inc.dataset.cartInc);if(f){f.qty++;saveCart();renderCart();}return;}
  const dec=e.target.closest('[data-cart-dec]');if(dec){const i=cart.findIndex(x=>lineKey(x)===dec.dataset.cartDec);if(i>-1){if(cart[i].qty>1)cart[i].qty--;else cart.splice(i,1);saveCart();renderCart();}return;}
  const rem=e.target.closest('[data-cart-remove]');if(rem){cart=cart.filter(i=>lineKey(i)!==rem.dataset.cartRemove);saveCart();renderCart();return;}
  const wr=e.target.closest('[data-wish-remove]');if(wr){wishlist=wishlist.filter(id=>id!==wr.dataset.wishRemove);saveWishlist();renderWishlistDrawer();renderProducts(currentFilter,currentSort);return;}
  const sr=e.target.closest('[data-search-id]');if(sr){closeSearch();openDetail(sr.dataset.searchId);return;}
  ...
  const sz=e.target.closest('#detailSizes [data-size]');if(sz){currentDetail.size=sz.dataset.size;document.querySelectorAll('#detailSizes .opt-btn').forEach(b=>b.classList.toggle('active',b===sz));return;}
  const cl=e.target.closest('#detailColors [data-color]');if(cl){currentDetail.color=cl.dataset.color;document.querySelectorAll('#detailColors .color-btn').forEach(b=>b.classList.toggle('active',b===cl));return;}
  ...
  const th=e.target.closest('[data-thumb]');if(th){showImageIndex(Number(th.dataset.thumb));return;}
  ...
  const sb=e.target.closest('[data-status]');if(sb){updateOrderStatus(sb.dataset.status);return;}
...
document.addEventListener('input',e=>{
  ...
  if(e.target.classList.contains('img-filename')){const slot=e.target.dataset.slot,val=e.target.value.trim();const prev=document.getElementById('prev-'+slot);if(!prev)return;if(val){const baseUrl=window.location.origin+window.location.pathname.replace(/[^/]*$/,'')+'images/';const src=val.startsWith('http')?val:baseUrl+val;prev.innerHTML=`<img src="${src}" alt="Preview" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 24 24%22 fill=%22none%22 stroke=%22%23999%22%3E%3Crect x=%223%22 y=%223%22 width=%2218%22 height=%2218%22 rx=%222%22/%3E%3C/svg%3E'">`;prev.classList.add('has-image');}else{prev.innerHTML=`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg><span>Image ${Number(slot)+1}</span>`;prev.classList.remove('has-image');}}
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeCart();closeWishlist();closeAuth();closeSearch();closeCheckout();closeSuccess();['detailModal','productModal','sizeGuideModal','orderDetailModal','adminOrderModal','adminUserModal','editModal'].forEach(id=>{const el=document.getElementById(id);if(el)el.classList.remove('show');});document.body.style.overflow='';}});
auth.onAuthStateChanged(user=>{const a=document.getElementById('accountBtn'),lo=document.getElementById('logoutBtn');if(a){if(user){const n=user.displayName||user.email.split('@')[0];a.textContent=n.split(' ')[0];a.title=user.email;}else{a.textContent='Login';a.title='';}}if(lo){lo.hidden=!user;lo.onclick=()=>auth.signOut().then(()=>{toast('Logged out');setTimeout(()=>location.href='index.html',500);});}if(page==='profile')loadProfile(user);});